...
impl DlcHandler {
    ...
    pub fn start_dlc_message_step(
        ...
        let dlc_message_step = match db::dlc_messages::get(conn, inbound_msg.message_hash)? {
...
impl DlcMessageStep {
    ...
    pub fn finish(&self, conn: &mut PgConnection, response: &Option<Message>) -> Result<()> {
        tracing::debug!("Marking the received message as processed");

        db::dlc_messages::insert(conn, self.inbound_msg.clone())?;
        ...
        if let Some(resp) = response {
            ...
            db::dlc_messages::insert(conn, outbound_msg)?;
            db::last_outbound_dlc_message::upsert(
                conn,
                &self.peer_id,
