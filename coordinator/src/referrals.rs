...
pub fn update_referral_status_for_user(
    ...
    if let Some(bonus_level) = &existing_status.bonus_status_type {
    ...
    let activated_referrals = referrals
        .iter()
        .filter(|referral| referral.referred_user_total_quantity.floor() > 0.0)
        .count();

    let referral_code = user.referral_code;
    if activated_referrals > 0 {
        let referral_tier = calculate_bonus_status_inner(bonus_tiers.clone(), activated_referrals)?;
        ...
        return Ok(ReferralStatus {
            referral_code,
            number_of_activated_referrals: activated_referrals,
