    ...
    async fn match_event(&self, event: Event) -> Result<()> {
        match event {
            ...
            Event::PaymentClaimed {
                ...
                handle_payment_claimed(&self.node, amount_msat, payment_hash, purpose);
            }
            Event::PaymentSent {
                payment_preimage,
                payment_hash,
                ...
                handle_payment_sent(&self.node, payment_hash, payment_preimage, fee_paid_msat)?;
            }
            ...
            Event::PaymentFailed { payment_hash, .. } => {
                handle_payment_failed(&self.node, payment_hash);
            }
            ...
            Event::PendingHTLCsForwardable { time_forwardable } => {
                handle_pending_htlcs_forwardable(
                    self.node.channel_manager.clone(),
                    time_forwardable,
                );
            }
            Event::SpendableOutputs { outputs } => {
                handle_spendable_outputs(&self.node, outputs)?;
            }
            Event::ChannelClosed {
                channel_id,
                reason,
            ...
            Event::DiscardFunding {
                ...
                handle_discard_funding(transaction, channel_id);
            }
            ...
            Event::HTLCHandlingFailed {
                ...
                handle_htlc_handling_failed(prev_channel_id, failed_next_destination);
            }
...
fn handle_htlc_handling_failed(
...
fn handle_discard_funding(transaction: bitcoin::Transaction, channel_id: [u8; 32]) {
...
fn handle_payment_failed<S>(node: &Arc<Node<S>>, payment_hash: PaymentHash)
where
    S: Storage,
{
    tracing::warn!(
...
async fn handle_funding_generation_ready<S>(
    node: &Arc<Node<S>>,
    user_channel_id: u128,
...
async fn handle_intercepted_htlc<S>(
    node: &Arc<Node<S>>,
    pending_intercepted_htlcs: &PendingInterceptedHtlcs,
...
async fn handle_intercepted_htlc_internal<S>(
    node: &Arc<Node<S>>,
    pending_intercepted_htlcs: &PendingInterceptedHtlcs,
    intercept_id: InterceptId,
    payment_hash: PaymentHash,
...
fn fail_intercepted_htlc(channel_manager: &Arc<ChannelManager>, intercept_id: &InterceptId) {
    tracing::error!(
        intercept_id = %intercept_id.0.to_hex(),
...
fn handle_pending_htlcs_forwardable(
