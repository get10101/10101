...
pub fn is_maker_running() -> bool {
    ...
    // On CI it's maker, on local it's target/debug/maker
    // added spaces to prevent matching on "maker" in a test name etc.
    is_process_running(" maker ") || is_process_running("target/debug/maker")
}
    ...
    std::str::from_utf8(&output.stdout)
        ...
        .any(|line| {
            if line.contains(process_name) {
                tracing::info!("Found process: {}", line);
                return true;
            }
            false
        })
