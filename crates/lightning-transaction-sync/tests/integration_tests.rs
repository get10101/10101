...
fn generate_blocks_and_wait(num: usize) {
    ...
    let _block_hashes_res = get_bitcoind()
        .client
...
fn wait_for_block(min_height: usize) {
    ...
    loop {
        ...
        header = exponential_backoff_poll(|| {
            get_electrsd()
                .trigger()
                .expect("failed to trigger electrsd");
            get_electrsd()
                .client
                .ping()
                .expect("failed to ping electrsd");
            get_electrsd()
                .client
                .block_headers_pop()
...
impl Confirm for TestConfirmable {
    fn transactions_confirmed(
        ...
        for (_, tx) in txdata {
            ...
            self.confirmed_txs
                .lock()
                .unwrap()
                .insert(txid, (block_hash, height));
            self.unconfirmed_txs.lock().unwrap().remove(&txid);
            self.events
                .lock()
                .unwrap()
                .push(TestConfirmableEvent::Confirmed(txid, block_hash, height));
        }
    }
    ...
    fn transaction_unconfirmed(&self, txid: &Txid) {
        ...
        self.events
            .lock()
            .unwrap()
            .push(TestConfirmableEvent::Unconfirmed(*txid));
    }
    ...
    fn best_block_updated(&self, header: &BlockHeader, height: u32) {
        ...
        *self.best_block.lock().unwrap() = (block_hash, height);
        self.events
            .lock()
            .unwrap()
            .push(TestConfirmableEvent::BestBlockUpdated(block_hash, height));
    }
    ...
    fn get_relevant_txids(&self) -> Vec<(Txid, Option<BlockHash>)> {
        self.confirmed_txs
            ...
            .unwrap()
            .iter()
            .map(|(&txid, (hash, _))| (txid, Some(*hash)))
            .collect::<Vec<_>>()
    }
}
